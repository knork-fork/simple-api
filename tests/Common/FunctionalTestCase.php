...
use App\Tests\Common\Attributes\RunsInTransaction;
use Error;
...
{
    public const BASE_URL = 'http://simpleapi-webserver';

    protected function tearDown(): void
    {
        parent::tearDown();

        // If class has RunsInTransaction attribute, restore the database after each test (snapshot point created in init-db.sh)
        if ($this->hasRunInTransactionAttribute()) {
            $this->rollback();
        }
    }
    ...
    private function hasRunInTransactionAttribute(): bool
    {
        $reflection = new ReflectionClass($this);

        return !empty($reflection->getAttributes(RunsInTransaction::class));
    }

    private function rollback(): void
    {
        // execute('scripts/restore_db_snapshot.sh');
        // not implemented
    }
}
